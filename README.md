...
**186 icons shipped** so far, across 24 categories — every one passes the draw-on contract validator. This is the pilot batch of a **10,049-concept taxonomy** ([`taxonomy.json`](./taxonomy.json)) the library fills over scheduled batches on the way to **10,000 drawn icons**. See [the roadmap](#the-10000-icon-roadmap) below.
...
**Grab a raw SVG file** — 186 of them in [`svg/`](./svg):
...
| File | What |
|---|---|
| [`svg/*.svg`](./svg) | one file per icon (186) |
...
**186 shipped** (this pilot batch), tier-1 first. New batches land on a schedule and extend `icons.js` / `index.json` / `svg/` in place — the taxonomy is the map, and it fills in over time.
